...
function Tables({ number, toggleModal }) {
  ...
  for (var i = 9 - number; i < 9; i++) {
    components.push(
      <div class="flex flex-col w-56 rounded-lg gap-2 h-80 bg-white/50 m-4 pt-4 pb-5 drop-shadow-2xl">
        <div class="flex w-full rounded-t-lg flex-auto justify-center self-center">
          <span class="text-center self-center content-center text-blue-900 pb-2 font-bold text-lg">Semester {i}</span>
        </div>
    ...
    <div className="App h-screen flex flex-col font-nunito">
      ...
      <div class="flex flex-row h-full bg-gradient-to-br from-cyan-600 to-violet-400">
        <div class="flex flex-col align-center justify-center basis-1/4 drop-shadow-2xl pl-7">
          <h1 class="text-4xl font-bold text-white justify-self-start"> Hey Ethan! </h1>
          <div class="flex flex-col align-center justify-center basis-1/4 drop-shadow-2xl mx-6 my-4 bg-slate-900/30 rounded-lg">
            <div class="text-xl m-6 self-center text-white">Enter your preferences:</div>
            <div class="text-sm self-center text-left w-48 text-white">Major</div>
            <select name="Major" class="mt-1 mb-3 w-48 self-center bg-white text-black text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5">
              <option disabled hidden selected class="text-black"></option>
              <option class="text-gray" value="CS">Computer Science (BSE)</option>
            </select>
            <div class="text-sm self-center text-left w-48 text-white">Minor</div>
            <select name="Minor" class="mt-1 mb-3 w-48 self-center bg-white  text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5">
              <option disabled hidden selected class="text-gray-900 text-sm "></option>
              <option value="CS">Computer Science</option>
            </select>
            <div class="text-sm self-center text-left w-48 text-white">Semesters Remaining</div>
            <select name="Semesters" class="w-48 mt-1 mb-3 self-center bg-white  text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5">
              <option disabled hidden selected class="text-gray-900 text-sm "></option>
            ...
            <div class="text-sm self-center text-left w-48 text-white">Interests</div>
            <input type="text" id="default-input" class="mt-1 mb-3 w-48 self-center bg-gray-50 border border-slate-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5" />
            <div class="text-sm self-center text-left w-48 text-white">Classes Taken</div>
            <MultiselectD class="multiselect self-center"></MultiselectD>
            <button type="submit" class="mt-3 mb-3 self-center text-white bg-medBlue hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-32 sm:w-auto px-5 py-2.5 text-center">Create a Course Plan</button>
          </div>
        </div>
        <div class="flex flex-col basis-3/4 bg-transparent pt-6 pb-6 relative ">
          ...
          <div class="grid grow grid-rows-2 grid-flow-col auto-cols-max place-content-evenly items-stretch justify-center content-center bg-red gap-4">
